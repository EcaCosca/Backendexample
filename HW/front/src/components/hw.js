...
function Hw(props) {
  ...
  const [plink, psetLink] = useState("");

  //GET
  useEffect(() => {
    axios.get("http://localhost:3001/hw").then((res) => {
      setData(res.data);
    });
  }, [data]);
  //POST
  const handleClick = () => {
    ...
    axios.post("http://localhost:3001/hw", obj).then((res) => {
      setData(res.data);
      psetTitle("");
      psetDescription("");
      psetLink("");
  ...
  const handelEdit = (e) => {
    ...
    axios.put("http://localhost:3001/hw", obj).then((res) => {
      setData(res.data);
      setTitle("");
      setDescription("");
      setLink("");
  ...
  const handelRemove = (id) => {
    axios.delete(`http://localhost:3001/hw/${id}`).then((res) => {
      setData(res.data);
    });
  };
    ...
    <div>
      <div className="flex justify-center">
        ...
        <input
          placeholder="URL ..."
          value={plink}
          onChange={(e) => psetLink(e.target.value)}
          className="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200"
        />
        <button
          ...
          return (
            ...
            >
              ...
              >
                ...
                <a className="my-4" href={e.link} style={{ color: "blue" }}>
                  Go to Github
                </a>
